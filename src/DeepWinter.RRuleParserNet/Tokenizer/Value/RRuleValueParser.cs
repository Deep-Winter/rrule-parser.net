    ...
    public class RRuleValueParser : IValueParser
    {
        ...
        private ValueWrapper GetUntilValue(string value)
        {
            try
            {
                try
                {
                    return new ValueWrapper(DateTime.ParseExact(value, UntilToken.DATE_FORMAT,
                        CultureInfo.InvariantCulture));
                }
                catch (System.Exception)
                {
                    return new ValueWrapper(DateTime.ParseExact(value, UntilToken.ALT_DATE_FORMAT,
                        CultureInfo.InvariantCulture));
                }
            }
            catch (System.Exception error)
        ...
        {
            var parts = value.Split(',').Select(str => str.Trim().ToUpperInvariant()).ToArray();
